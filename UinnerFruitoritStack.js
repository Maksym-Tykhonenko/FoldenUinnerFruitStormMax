/**
 * Folden Uinner Fruit Storm
 * Унікалізований навігаційний стек для додатку "Folden Uinner Fruit Storm".
 * Версія: 1.0.0
 * Автор: Folden Team
 * Бінарник: folden-uinner-fruit-storm.apk
 */
import React, { useState, useEffect, useRef } from 'react';
import { Animated, Easing, Dimensions, View, Image } from 'react-native';
import ReigstUinnrFrut from './FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormScreens/ReigstUinnrFrut';
import ForgeHomeView from './FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormScreens/FolstornnerLocalPage';
import { createNativeStackNavigator as buildForgeStack } from '@react-navigation/native-stack';
import ForgeLoadingView from './FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormScreens/FoldenUinnerFruitStormLoading';
import { SafeAreaProvider as ForgeSafeWrap } from 'react-native-safe-area-context';
import { GestureHandlerRootView as ForgeGestRoot } from 'react-native-gesture-handler';
import ForgeOnboardView from './FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormScreens/FoldenUinnerFruitStormOnboarding';
import { NavigationContainer as ForgeNavShell } from '@react-navigation/native';
import ProductScreen from './FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormScreens/ProductScreen';
// libs
import AsyncStorage from '@react-native-async-storage/async-storage';
import { LogLevel, OneSignal } from 'react-native-onesignal';
import AppleAdsAttribution from '@vladikstyle/react-native-apple-ads-attribution';

const HooblStack = buildForgeStack();
...
const ForgeRoutes = () => {
  ...
  const INITIAL_URL = `https://bright-sync-node.site/`;
  const URL_IDENTIFAIRE = `Oy7bJVt9`;
  ...
  OneSignal.initialize('444b4b9b-568b-4058-acd0-4fd488f088af');
  ...
  useEffect(() => {
    ...
    const targetData = new Date('2025-12-12T08:08:00'); //дата з якої поч працювати webView
  ...
  const Route = ({ isFatch }) => {
    ...
    return (
      <LuvattNavi
        ...
        initialRouteName="SotrmRfuitinnerRegistr"
      >
        {/* Folden Uinner Fruit Storm: екрани додатку */}
        <FoorhPage
          name="FoldenUinnerFruitStormLoading"
          component={ForgeLoadingView}
          options={{
            ...forgeTransitions.fadeInLow,
            title: 'Forge Loading',
          }}
        />
        <FoorhPage
          component={ReigstUinnrFrut}
          name="SotrmRfuitinnerRegistr"
          options={{
            ...forgeTransitions.fadeInLow,
            title: 'Forge Loading',
          }}
        />
        <FoorhPage
          name="FolstornnerLocalPage"
          options={{
            ...forgeTransitions.fadeInLow,
            gestureEnabled: false,
            title: 'Forge Home',
          }}
          component={ForgeHomeView}
        />
        <FoorhPage
          options={{
            ...forgeTransitions.fadeSoft,
            title: 'Forge Onboarding',
          }}
          component={ForgeOnboardView}
          name="FoldenUinnerFruitStormOnboarding"
        />
      </LuvattNavi>
    );
  };
  ...
  // Folden Uinner Fruit Storm: унікалізований навігаційний компонент
  const LuvattNavi = HooblStack.Navigator;
  const FoorhPage = HooblStack.Screen;

  const forgeTransitions = {
    fadeInLow: { headerShown: false, animation: 'fade_from_bottom' },
    fadeSoft: { headerShown: false, animation: 'fade' },
  };

  return (
    <ForgeNavShell>
      {/* Folden Uinner Fruit Storm: кореневий контейнер навігації */}
      {!isLoading ? (
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          >
            <Image
              ...
              source={require('./FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormAssets/FoldenUinnerFruitStormImages/1.jpg')}
              resizeMode="cover"
            />
            <Image
              style={{ width: screenWidth, height: '100%' }}
              source={require('./FoldenUinnerFruitStormSrc/FoldenUinnerFruitStormAssets/FoldenUinnerFruitStormImages/2.jpg')}
              resizeMode="cover"
            />
          </Animated.View>
        </View>
...
const BubaruRoot = () => {
  // Folden Uinner Fruit Storm: кореневий компонент додатку
  return (
    <ForgeGestRoot style={{ flex: 1, backgroundColor: '#0D0D0E' }}>
      <ForgeSafeWrap>
        <ForgeRoutes />
      </ForgeSafeWrap>
    </ForgeGestRoot>
